    ...
    <div style={wrap}>
      ...
      <Section title="各タブの使い方" hint="タブ別の機能解説">
        ...
        <Block label="作曲">
          ...
          <b>3 声部構成</b>: リード（モーラ同期の旋律）+ ベース（拍単位の低音）+ コード（小節単位の和音 pad）を AI が同時に生成。それぞれ別の synth で同時発音し、単調な繰り返しを避ける指示も AI に渡している。
          <b>自動保存</b>: 生成したメロディは組詩 + セクションごとに localStorage（端末ローカル）へ自動保存。同じセクションを開けば復元され、再生成したい時は <b>クリア</b> ボタンで破棄。
          <b>テンポ / キー（C 〜 B）/ 旋法（長調・短調・ペンタ・陰旋法・陽旋法・ドリアン）</b>を指定可能（自動なら AI に任せる）。
          <b>ランダム度 0〜4</b> で生成の保守的さと再生時の揺らぎ（jitter / 装飾音 / 追加ハーモニー）を制御。
          <br /><br />
          <b>生成方式の切替:</b>
